...
from nets.FHAMN_training import CE_Loss, Dice_loss, Focal_Loss, weights_init
...
def fit_one_epoch(model_train, model, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch,
                  cuda, dice_loss, focal_loss, cls_weights, num_classes,save_period):
    total_loss = 0
    total_f_score = 0
    ...
    if (epoch + 1) % save_period == 0 or epoch + 1 == Epoch:
        torch.save(model.state_dict(), 'ISPRS/ep%03d-loss%.3f-val_loss%.3f.pth' % (
            (epoch + 1), total_loss / (epoch_step + 1), val_loss / (epoch_step_val + 1)))

    if len(loss_history.val_loss) <= 1 or (val_loss / epoch_step_val) <= min(loss_history.val_loss):
        print('Save best model to best_epoch_weights.pth')
        torch.save(model.state_dict(), os.path.join("ISPRS/best_epoch_weights.pth"))
